...
pufferfish=$(curl -H "Accept: application/vnd.github.v3+json" https://api.github.com/repos/Yive/Pufferfish/compare/$1...$2 | jq -r '.commits[] | "Yive/Pufferfish@\(.sha[:7]) \(.commit.message | split("\r\n")[0] | split("\n")[0])"')
